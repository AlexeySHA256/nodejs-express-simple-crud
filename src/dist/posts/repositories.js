...
export class PostsRepositoryImpl {
    ...
    async getPost(options) {
        ...
        return prisma.post.findUniqueOrThrow({ where: { id: options.id }, include: { author: options.withAuthor, comments: options.withComments ? { include: { author: true } } : undefined } })
            .then((post) => {
            return Post.fromObject({ ...post, author: User.fromObject(post.author), comments: post.comments.map((comment) => new Comment({ ...comment, author: User.fromObject(comment.author) })) });
        })
            .catch((err) => {
