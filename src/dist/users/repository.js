...
export class TokensRepositoryImpl {
    async createToken(data) {
        return prisma.token.create({ data })
            .then((token) => new Token({ ...token, scope: token.scope, plainText: "" }));
    }
    async generateAndCreateToken(userId, ttlMs, scope = TokenScopes.AUTHORIZATION) {
    ...
    async getToken(options) {
        ...
        return prisma.token.findUniqueOrThrow({ where: { hash: options.hash, scope: options.scope }, include: { user: options.withUser } })
            .then((token) => new Token({ ...token, scope: token.scope, user: User.fromObject(token.user), plainText: "" }))
            .catch((err) => {
            if (err instanceof Prisma.PrismaClientKnownRequestError && err.code === NotFoundErrCode) {
